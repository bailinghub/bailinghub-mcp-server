...
**未发布源码候选：**当前源码还支持由宿主明确选择同一 Hub 上的多个业务系统，每个系统使用自己的
已授权连接。它需要配套 Core 能力与宿主接入；公开 npm `0.4.0` 和 Core `0.6.1` 尚不包含跨系统对话
归档。开发接入见[跨系统源码候选契约](docs/AGENT_CLIENT_SDK.zh-CN.md#跨系统源码候选)。

源码候选还支持业务后端提供的 **授权主体名称**，例如组织、团队、项目、账号或经营场所。
授权成功后，宿主可自动显示实际确认的对象，并独立保存内部连接标识。此能力需要配套 Core 和
宿主候选，不能只凭公开包版本号判断已经支持。详见[授权主体展示信息](docs/AGENT_CLIENT_SDK.zh-CN.md#授权主体展示信息源码候选)。
